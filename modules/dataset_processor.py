...
class Processor(object):
    ...
    def get_dataset(self) -> Dataset:
        ...
        )  # dataset name needs to be set in processor class
        oracle_provenance_str = "_oracle_provenance" if self.oracle_provenance else ""
        out_folder = os.path.join(
            f"{self.out_folder}",
            f"{self.dataset_name}_{self.split}{oracle_provenance_str}",
        )
...
class MMLU(Processor):
    ...
    def process(self):
        ...
        dataset = datasets.load_dataset(
            hf_name, hf_query_or_doc_name, num_proc=self.num_proc
        )[self.split]
        ...
        # use "sample_id" ?
        dataset = dataset.map(
            lambda example, idx: {"id": str(idx), **example}, with_indices=True
        )
...
class MsMarcoQueries(Processor):
    ...
    def process(self):
        queries_d = json.load(
            open(
                "/gfs-ssd/user/tformal/neural_search/MSMARCO/dev_queries_collection/dev_queries.json"
            )
        )  # super hard-coded path, see how to do properly
        ids, queries = zip(*queries_d.items())
        dataset = datasets.Dataset.from_dict(
            {"id": ids, "content": queries}
        )  # no need for split?
        return dataset
...
class KILT100w(Processor):
    ...
    def process(self):
        ...
        def map_100w(sample, num_words=100):
            ...
            passages = [
                x.strip() for x in sample["text"]["paragraph"] if "BULLET::::" not in x
            ]
            doc = " ".join(passages)
        ...
        def map_predefined_paragraphs(sample):
            ...
            title = sample["wikipedia_title"]
            paragraphs = [
                title + ". " + el.replace("Section::::", "Section:")
                for el in sample["text"]["paragraph"]
            ]
        ...
        if self.oracle_provenance:
            ...
            dataset = datasets.Dataset.from_dict(
                {"content": paragraphs, "wikipedia_id": wiki_ids}
            )
            dataset = dataset.map(
                lambda example, idx: {"id": str(idx), **example}, with_indices=True
            )
...
class Wiki_monolingual_100w(Processor):
    ...
    def process(self):
        ...
        dataset = datasets.load_dataset(hf_name, subset, num_proc=self.num_proc)[
            self.split
        ]
...
class PubMed2023(Processor):
    ...
    def process(self):
        hf_name = "ncbi/pubmed"
        dataset = datasets.load_dataset(
            hf_name, num_proc=self.num_proc, trust_remote_code=True
        )[self.split]

        def map_fn(example):
            example["content"] = (
                f"{example['MedlineCitation']['Article']['ArticleTitle']}: {example['MedlineCitation']['Article']['Abstract']['AbstractText']}"
            )
...
class Wikipedia2023_full(Processor):
    ...
    def process(self):
        hf_name = "wikimedia/wikipedia"
        dataset = datasets.load_dataset(hf_name, "20231101.en", num_proc=self.num_proc)[
            self.split
        ]
...
class MsMarcoCollection(Processor):
    ...
    def process(self):
        ...
        dataset = datasets.load_dataset(
            hf_name, "docs", num_proc=self.num_proc
        )  # no need for split?
...
class UT1Queries(Processor):
    ...
    def process(self):
        ...
        tests_directory = os.path.join(
            os.path.dirname(os.path.dirname(script_path)), "tests"
        )
...
class MergedDocDataset(Processor):
    def __init__(
        self, out_dataset_name, in_dataset_names, in_dataset_splits, *args, **kwargs
    ):
    ...
    def process(self):
        raise NotImplementedError(
            "Datasets for merging should be preprocessed independently before runing experiments with the merged dataset."
        )
    ...
    def get_dataset(self):
        ...
        for dataset_name, split in zip(self.in_dataset_names, self.in_dataset_splits):
            in_folder = os.path.join(
                f"{self.out_folder}", f"{dataset_name}_{split}{oracle_provenance_str}"
            )
            if not os.path.exists(in_folder):
                raise ValueError(f"Dataset {in_folder} not found")
            dataset = datasets.load_from_disk(in_folder)
            dataset = dataset.map(
                partial(prepend_label, label=dataset_name), num_proc=self.num_proc
            )
...
class ProcessDatasets:
            ...
            for query_or_doc in datasets[split]:
                if datasets[split][query_or_doc] != None:
                    ...
                    processor = instantiate(
                        ...
                        oracle_provenance=(
                            oracle_provenance if query_or_doc == "doc" else False
                        ),
                        shuffle_labels=(
                            shuffle_labels if query_or_doc == "query" else False
                        ),
                    )
    ...
    def check_instantiate(
        datasets, out_folder="datasets", num_proc=1, overwrite=False, debug=False
    ):
        processed_datasets = defaultdict(dict)
        for split in datasets:
            for query_or_doc in datasets[split]:
