...
parser.add_argument('--model', type=str, required=False, default="Qwen/Qwen3-8B", help="HuggingFace model name")
...
# labeler LLM
# the code was run with "Qwen/Qwen3-8B"
# in case of using another model check if it works well with the reference format used throughout the code.

tokenizer = AutoTokenizer.from_pretrained(args.model)
model = LLM(model=args.model, tensor_parallel_size=torch.cuda.device_count(), dtype=torch.bfloat16, 
            gpu_memory_utilization=0.9, enforce_eager=True, kv_cache_dtype="fp8_e5m2")
...
for q, cntx in zip(gen_dataset["query"][::args.skip], gen_dataset['doc'][::args.skip]):
    ...
    sents = [sent.text.strip() for sent in nlp(cntx).sents]
